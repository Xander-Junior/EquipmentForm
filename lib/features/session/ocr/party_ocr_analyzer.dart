...
class PartyOcrAnalyzer {
  ...
  _TitleExtraction _extractTitles(
    ...
    suggestions.sort((a, b) => b.score.compareTo(a.score));
    return _TitleExtraction(
        suggestions: suggestions, consumedLineIndexes: consumed);
  }
  ...
  List<PartyOcrFieldSuggestion> _extractDepartments(
    ...
    if (fieldDept is String && fieldDept.trim().isNotEmpty) {
      final normalized =
          _canonicalDepartment(fieldDept) ?? _normalizeDepartment(fieldDept);
      addSuggestion(
        value: normalized,
        score: 0.84,
        evidence: fieldDept,
  ...
  double _scoreTitle(String normalized) {
    ...
    if (lower.contains('analyst') || lower.contains('specialist')) {
      score += 0.04;
    }
  ...
  bool _isLikelyTitleLine(String text) {
    ...
    if (lower.contains('@')) return false;
    if (lower.contains('department') || lower.contains('division')) {
      return false;
    }
    final keywords = {
      'applications',
  ...
  String? _extractHighlight(String text, String inferred) {
    ...
    if (lowerInferred.contains('information technology') &&
      ...
      return text.substring(start,
          math.min(start + 'Information Technology'.length, text.length));
    }
    return null;
  }
