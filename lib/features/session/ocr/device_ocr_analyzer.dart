...
class DeviceOcrAnalyzer {
  ...
  List<DeviceOcrFieldSuggestion> _extractAssetTags(
    ...
    final patterns = [
      ...
      RegExp(r'([0-9]{6,10})',
          caseSensitive: false), // Numeric asset tags (6-10 digits)
      ...
      for (final line in lines) {
        ...
        for (final match in pattern.allMatches(line)) {
          final value =
              (match.groupCount > 0 ? match.group(1) : match.group(0)) ?? '';
          if (value.length >= 3 &&
              !_isIgnorableIdentifier(value) &&
              _isValidServiceTag(value, line)) {
            addSuggestion(
              value: value,
              score: baseScore,
              evidence: line,
  ...
  String _normalizeModelValue(String input) {
    ...
    if (normalized != null && normalized.isNotEmpty && normalized != input) {
      final code = normalized.trim().toUpperCase();
      final lower = input.trim().toLowerCase();
      final hasDellPrefix = lower.startsWith('dell');
      return '${hasDellPrefix ? 'Dell ' : ''}Latitude $code'.trim();
    }
  ...
  bool _isValidServiceTag(String value, String line) {
    ...
    if (value.length >= 3 && value.length <= 10 && (hasLetters || hasNumbers)) {
      ...
      if (trimmedLine == value ||
          trimmedLine.endsWith(': $value') ||
          trimmedLine.endsWith(':$value')) {
        return true;
      }
    }
