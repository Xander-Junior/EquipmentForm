...
final _assetTagPattern = RegExp(
    r'^(ACC|TAK|LON)\s*-\s*(LT|MB)\s*-\s*(\d{4,6})$',
    caseSensitive: false);
...
String? validateAssetTag(String? input) {
  ...
  return parseAssetTag(input) == null
      ? 'Asset tag must match ACC-LT-12345 or TAK-MB-1234'
      : null;
}
...
String? validateWarrantyExpiry(String? input) {
  ...
  return normalizeWarrantyExpiry(input) == null
      ? 'Warranty expiry must be a valid date (DD/MM/YYYY)'
      : null;
}
