...
class PdfTypography {
  ...
  pw.TextStyle bodyMedium({bool boldWeight = false}) => pw.TextStyle(
      font: boldWeight ? bold : regular, fontSize: 11, height: 1.2);
  pw.TextStyle bodySmall({bool boldWeight = false}) => pw.TextStyle(
      font: boldWeight ? bold : regular, fontSize: 10, height: 1.2);
}
...
class PdfRenderer {
  ...
  Future<void> _ensureAssets() async {
    ...
    if (_fieldMap == null) {
      final yamlString =
          await rootBundle.loadString('eqpt_template_ref/field_map.yaml');
  ...
  Future<Uint8List> render({
    ...
    final config =
        _fieldMap?[session.formType.name] as Map<String, dynamic>? ?? const {};
        ...
        build: (context) {
          return pw.Column(
            ...
            children: [
              ...
              metadata,
              pw.SizedBox(height: 16),
              _partySection(state.party,
                  config['party'] as Map<String, dynamic>?, typography),
              pw.SizedBox(height: 16),
              _equipmentTable(state.equipment.primaries,
                  config['table'] as Map<String, dynamic>?, typography),
              pw.SizedBox(height: 16),
              _workflowSection(session.formType, state.workflow,
                  config['workflow'] as Map<String, dynamic>?, typography),
            ],
          );
        },
      ),
  ...
  void _buildReplaced(
    ...
    final metadata = _metadataTable(session, generated, typography);
    final newDevices =
        state.equipment.primaries.where((d) => !d.isReplacementOld).toList();
    final oldDevices =
        state.equipment.primaries.where((d) => d.isReplacementOld).toList();
                ...
                children: [
                  pw.Expanded(
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Text(config['newLabel']?.toString() ?? 'NEW DEVICE',
                  ...
                  pw.Expanded(
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Text(config['oldLabel']?.toString() ?? 'OLD DEVICE',
                            style: typography.titleMedium()),
                        pw.SizedBox(height: 8),
                        _equipmentTable(
                            oldDevices,
  ...
  pw.Widget _metadataTable(
      Session session, DateTime generated, PdfTypography typography) {
  ...
  pw.Widget _partySection(PartySessionState party, Map<String, dynamic>? config,
      PdfTypography typography) {
  ...
  pw.Widget _equipmentTable(List<PrimaryDeviceState> devices,
      Map<String, dynamic>? config, PdfTypography typography) {
    final leftHeader =
        config?['leftHeader']?.toString() ?? 'Equipment Make / Model';
    final rightHeader =
        config?['rightHeader']?.toString() ?? 'Equipment Serial / Details';
  ...
  List<String> _deviceDetails(PrimaryDeviceState device) {
    ...
    if (device.type == PrimaryDeviceType.laptop) {
      if ((device.assetTag ?? '').isNotEmpty) {
        details.add('ASSET-TAG: ${device.assetTag}');
      }
      if ((device.serviceTag ?? '').isNotEmpty) {
        details.add('SERVICE TAG: ${device.serviceTag}');
      }
      if ((device.warrantyExpiry ?? '').isNotEmpty) {
        details.add('W.E.: ${device.warrantyExpiry}');
      }
    } else {
      if ((device.imei ?? '').isNotEmpty) details.add('IMEI: ${device.imei}');
      if ((device.serialNumber ?? '').isNotEmpty) {
        details.add('SERIAL: ${device.serialNumber}');
      }
      if ((device.assetTag ?? '').isNotEmpty) {
        details.add('ASSET-TAG: ${device.assetTag}');
      }
    ...
    switch (formType) {
      case FormType.received:
        rows.add([
          ...
          workflow.dateReceived != null
              ? DateFormat('yyyy-MM-dd').format(workflow.dateReceived!)
              : '—',
        ]);
        break;
      case FormType.returned:
        rows.add([
          ...
          workflow.dateReturned != null
              ? DateFormat('yyyy-MM-dd').format(workflow.dateReturned!)
              : '—',
        ]);
        rows.add([
