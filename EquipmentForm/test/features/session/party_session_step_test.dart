...
class _TestSectionCaptureWizard extends SectionCaptureWizard {
  _TestSectionCaptureWizard(
    ...
    PartyOcrReviewPresenter? presenter,
  }) : super(ref, formType, presenter: presenter);
...
void main() {
  ...
  Future<void> ensurePartyFieldVisible(WidgetTester tester, Finder finder) async {
  ...
  Future<ProviderContainer> _pumpPartySession(
    WidgetTester tester,
    ...
    final container = ProviderContainer(
      overrides: [
        sessionControllerProvider(formType)
  ...
  testWidgets('linking copies values and shows indicator', (tester) async {
    ...
    await _pumpPartySession(tester, controller);
  ...
  testWidgets('prefilled email selects all on first focus only once', (tester) async {
    final controller = SessionController(
      formType: FormType.received,
      currentUser: demoUserProfile,
    );
    ...
    final emailEditableFinder = find.descendant(of: emailField, matching: find.byType(EditableText));
    ...
    expect(editable.controller.selection.extentOffset, editable.controller.text.length);
    ...
    final nameEditableFinder = find.descendant(of: nameField, matching: find.byType(EditableText));
    ...
    expect(editable.controller.selection.baseOffset, editable.controller.selection.extentOffset);
    expect(editable.controller.selection.baseOffset, editable.controller.text.length);
  });
  ...
  testWidgets('blur with trailing @ appends tullowoil.com and validator updates', (tester) async {
    final controller = SessionController(
      formType: FormType.received,
      currentUser: demoUserProfile,
    );
  ...
  testWidgets('party section wizard applies fixture and supports undo', (tester) async {
    late final OcrResult ocrResult;
    late final File image;

    await tester.runAsync(() async {
      final fixtureFile = File('test/fixtures/ocr/party/otuko_john_teye.json');
      final fixtureData = jsonDecode(await fixtureFile.readAsString())
    ...
    PartyOcrAnalysis? capturedAnalysis;
    ...
    final presenter = (
      ...
      final deptSuggestion = analysis.primarySuggestionFor(PartyField.department)!;
    ...
    final container = await _pumpPartySession(
      tester,
      controller,
      overrides: [
    ...
    final appliedMember = container
        ...
        .party
        .members[PartyRole.preparedBy]!;
    final appliedMemberName =
        appliedMember.fields[PartyField.name]!.value;
    final appliedMemberDept =
        appliedMember.fields[PartyField.department]!.value;
    final appliedMemberEmail =
        appliedMember.fields[PartyField.email]!.value;
    final st =
        container.read(sessionControllerProvider(FormType.received));
    final appliedName = st.party.members[PartyRole.preparedBy]!
        .fields[PartyField.name]!
        .value;
  ...
  testWidgets('party section wizard repairs Francis OCR data', (tester) async {
    ...
    final presenter = (
      ...
      for (final entry in controllers.entries) {
        final suggestion =
            analysis.primarySuggestionFor(entry.key) ?? initialSuggestions[entry.key];
        if (suggestion != null) {
