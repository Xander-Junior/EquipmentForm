...
void main() {
  final analyzer = PartyOcrAnalyzer();
  ...
  test('hyphenated names normalise with proper casing', () {
    ...
    final allNames =
        analysis.nameSuggestions.map((s) => '${s.value}:${s.reason}:${s.score}').toList();
    expect(
  ...
  test('supply chain inference prefers canonical labels', () {
    ...
    final department = analysis.primarySuggestionFor(PartyField.department)!.value;
  ...
  test('confidence boosts when multiple sources agree', () {
    ...
    expect(department.confidence, anyOf(PartyOcrConfidence.high, PartyOcrConfidence.medium));
  });
}
